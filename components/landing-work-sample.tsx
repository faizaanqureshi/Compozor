  ...
  return (
    <figure className="min-w-0">
      <Tabs
        ...
        className={`${depth.workbench} gap-0 overflow-hidden rounded-xl border border-sidebar-foreground/20`}
      >
        <div className={`${glass.dark} flex flex-col justify-between gap-4 border-b px-5 py-5 sm:flex-row sm:items-center sm:px-8`}>
          <div className="flex items-center gap-3">
            <FileSpreadsheet
              className="size-4 text-marketing-brass"
              aria-hidden
            />
          ...
          <TabsList
            ...
            className={`${glass.switch} justify-start`}
          >
            <TabsTrigger
              value="sample"
              className="flex-none px-0 text-xs text-sidebar-foreground/80 after:bg-marketing-brass hover:text-sidebar-foreground data-active:text-marketing-brass"
            >
              Reference layout
            </TabsTrigger>
            <TabsTrigger
        ...
        <div className="grid gap-8 p-5 sm:p-8 lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-10">
          <aside className="grid content-start gap-7 sm:grid-cols-2 lg:grid-cols-1 lg:gap-9">
            <div>
              <p className="text-[0.625rem] uppercase tracking-widest text-marketing-brass">
                01 / The material
              </p>
              <div className="mt-4 space-y-4">
                {[
                  ...
                  <div key={name} className="flex gap-3">
                    <FileText
                      className="mt-0.5 size-3.5 shrink-0 text-sidebar-foreground/70"
                      aria-hidden
                    />
                    <div>
                      <p className="text-xs">{name}</p>
                      <p className="mt-1 text-[0.625rem] leading-relaxed text-sidebar-foreground/70">
                        {detail}
                      </p>
                    </div>
                  </div>
            ...
            <div>
              ...
              <blockquote className="mt-4 border-l border-marketing-brass/60 pl-4 text-sm leading-relaxed text-sidebar-foreground/90">
                “Summarize expenses by category. Show the spending trend, retain
                our format, and include source references and review notes.”
              </blockquote>
              <div className="mt-6 space-y-2 text-[0.625rem] text-sidebar-foreground/80">
      ...
      <figcaption className="mt-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <p className="max-w-xl text-xs leading-relaxed text-sidebar-foreground/75">
        ...
        <Button
          ...
          variant="ghost"
          className="shrink-0 self-start gap-3 text-sidebar-foreground hover:bg-sidebar-foreground/10 hover:text-sidebar-foreground sm:self-auto"
        >
          {view === "sample"
